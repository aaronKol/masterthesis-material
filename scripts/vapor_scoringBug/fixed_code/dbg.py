""" 
This code belongs to VAPOR by southgate. The owning repository is linked here:
...
class wDBG():
    ...
    def complete_query(self, sr, kmers=None, string=None, debug=False):
        ...
        amount_gaps = sum([gapr - gapl for gapl, gapr in gaps])
        est_query_cov = 1 - amount_gaps/len(filled_deque_array)
        #est_pid = len(nonzeros)/len(filled_deque_array)
        sr.est_query_cov = est_query_cov
        score = sum(nonzeros)
        sr.score = score
        return sr
    ...
    def query(self, kmers, min_kmer_prop, debug=False):
        ...
        # Sum, also get estimated pid
        nonzeros = [i for i in filled_deque_array if i > 0]
        amount_gaps = 1 - sum([gapr - gapl for gapl, gapr in gaps])
        est_query_cov = amount_gaps/len(filled_deque_array)
        #est_pid = len(nonzeros)/len(filled_deque_array)
        sr.est_query_cov = est_query_cov
        score = sum(nonzeros)
        sr.score = score
    ...
    def classify(self, seqs, seqsh, min_kmer_prop, top_seed_frac, debug_query=None, low_mem=False):
        ...
        # Sort the results
        results = sorted(scores, key = lambda x: x.score * x.est_query_cov, reverse=True)
        if debug_query != None:
            # For debugging
